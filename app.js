...
function computeTowerStats(typeIndex, level = 1){
    ...
    // Баланс: лёгкий рост дальности/скорости и более заметный рост урона
    const range = Math.round(base.range * (1 + 0.12 * step));
    const damage = Math.round(base.damage * (1 + 0.35 * step));
    const firerate = +(base.firerate * (1 + 0.18 * step)).toFixed(2);
...
function ensureTowerRuntimeFields(tower){
    ...
    if (tower.spent == null) tower.spent = towerTypes[tower.type].cost;
...
function getUpgradeCost(tower){
    ...
    // 1->2 дешевле, 2->3 дороже
    return Math.round(baseCost * (lvl === 1 ? 0.6 : 0.9));
}

function getSellRefund(tower){
    const spent = (tower && tower.spent != null) ? tower.spent : 0;
    return Math.max(0, Math.round(spent * 0.7));
}
...
function startWave() {
    ...
    // очередь спавна — теперь привязана к "внутреннему" времени симуляции (для паузы/ускорения)
    gameState.spawnQueue = [];
    ...
    for (let i = 0; i < numEnemies; i++) {
        gameState.spawnQueue.push({
            ...
            speed: 1.2 + gameState.wave * 0.15, // px/кадр (будет масштабировано dt)
...
function gameLoop(ctx, canvas) {
    ...
    gameState.enemies = gameState.enemies.filter(enemy => {
        ...
        if (enemy.health <= 0) {
            ...
            if (gameState.waveStats) gameState.waveStats.kills++;
